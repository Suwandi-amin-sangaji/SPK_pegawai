<?php
	class DB{
		...
		function bukaKoneksi(){
			try{
				$this->koneksi = new PDO("mysql:host=localhost;dbname=spk_pegawai","root","root", array(PDO::ATTR_PERSISTENT=>TRUE));
		...
		function LoginManager($username, $password){
			try{
				$sql = $this->bukaKoneksi()->prepare("select * from manager where username = :username and password = :password");
				$sql->bindParam(':username', $username);
				$sql->bindParam(':password', $password);
				$sql->execute();
				return $sql;
